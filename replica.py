...
class Replica(object):
    ...
    def handle_TimeOut(self, m):
        print 'handle_TimeOut', m
        self.view += 1
        if (self.view == self.uid):
            self.beProposor()

    def handle_Request(self, m):
        print 'handle_request', m
        if self.view == self.uid:
            if self.num_followers >= self.f + 1:
